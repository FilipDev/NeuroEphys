...
class PositionMatrix:
    ...
    def _read_positions(self, start, end):
        ...
        for i in range(len(data)):
            ...
            if i > 0:
                ...
                self.distances.append(math.sqrt((datum[1] - data[i - 1, 1]) ** 2 +
                                                (datum[2] - data[i - 1, 2]) ** 2))
            data[i, 0] = round(data[i, 0] / 10 ** 4,
                               self.precision)  # Converts time to seconds, and rounds to precision
    ...
    def smooth_values(self):
        ...
        self.data_smoothed = analyzer.apply_kernel(self.data[:, 1], fps, 0.3, kernel_type='gaussian'), \
                             analyzer.apply_kernel(self.data[:, 2], fps, 0.3, kernel_type='gaussian')
    ...
    def find_rotational_velocity(self):
        ...
        min_rotation = 4
        prior = 0
        print("Calculating rotational velocities", np.max(self.angular_velocities))
        for i in range(len(self.angular_velocities)):
            print((360 / 2 / np.pi) * self.angular_velocities[i])
            #self.rotational_velocities[i] = self.angular_velocities[i]

            # TODO: Rework
            # If the number of pixels moved is greater than the threshold
            if self.distances[i] > distance_threshold:
                self.rotational_velocities[i] = (360 / 2 / np.pi) * self.angular_velocities[i]
                ...
                self.rotational_velocities[i] = prior  # TODO: Account for this NAN
